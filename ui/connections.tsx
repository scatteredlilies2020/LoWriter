...
import type { AuthMode, Connection, Dialect, Route } from '../src/shared.ts';
...
export function ConnectionForm({ current, profiles, unlocked, onSaved }: { current: Connection | null; profiles: Connection[]; unlocked: boolean; onSaved: () => Promise<unknown> }) {
  ...
  const draft = () => ({ provider, ...resolved, variant, region, model, auth, name, route, proxyUrl, apiKey: key, keyMode: clearKey ? 'clear' : 'keep' });
  ...
  return <section class="connection-form">
    <p>Choose a built-in provider—its address is automatic—or use a custom proxy in any API category. Writing and coding share the active text connection. Speech is saved separately.</p>
    ...
    <form onSubmit={e => { e.preventDefault(); void run(async signal => { try { await post('connection', draft(), signal); await onSaved(); setNotice(resolved.dialect === 'speech' ? 'Speech connection saved; writing connection unchanged.' : 'Connection saved and selected. Send a message to test your account/model.'); } finally { setKey(''); } }); }}>
      <fieldset disabled={busy}>
        ...
        <p class="hint">Selecting a provider sends nothing. Load models contacts only the selected endpoint. Catalog entries do not guarantee account access or tool support. Requests keep the selected route; no redirects or automatic paid retries.</p>
        <button class="primary" disabled={!unlocked}>{resolved.dialect === 'speech' ? 'Save speech connection' : 'Save connection'}</button>
